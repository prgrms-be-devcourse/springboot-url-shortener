...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<ErrorResponse> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) {
        ...
        cause = format("필드에 맞지 않는 입력값을 입력하였습니다. (필드: {0}, 입력 값: {1})", requireNonNull(fieldError).getField(), fieldError.getRejectedValue());
    ...
    protected ResponseEntity<ErrorResponse> handleTypeMismatch(TypeMismatchException ex) {
        cause = format("잘못된 타입이 입력되었습니다. (기대 타입: {0}, 입력 타입: {1})", requireNonNull(ex.getRequiredType()).getSimpleName(), ex.getValue());
    ...
    protected ResponseEntity<ErrorResponse> missingServletRequestParameterException(HttpServletRequest request, MissingServletRequestParameterException ex) {
        cause = format("필요한 파라미터가 존재하지 않습니다. (필요한 파라미터: {0}, 요청된 파라미터: {1})", ex.getParameterName(), request.getQueryString());
